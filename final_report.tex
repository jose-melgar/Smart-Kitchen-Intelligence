...
\begin{titlepage}
    ...
    % Marcador de espacio para logotipo (puedes descomentar la línea si tienes la imagen)
    % \includegraphics[width=0.35\textwidth]{logo_upc.png} \\[1cm]
    \vspace*{1.5cm}
...
% SECCIÓN 3
% =========================================================================
\section{Fuentes de Datos y Condiciones de Acceso (Dataset Sources)}
...
\subsubsection{Matriz de Características Refinada: \texttt{feature\_matrix.npy} ($72,000 \times 61$)}
\begin{itemize}
    \item \textbf{Filas (72,000):} Array indexado que representa cada estado de sesión transaccional desagregado por evento temporal.
    \item \textbf{Columnas (61):} Vector Denso compuesto por: 4 variables de nutrientes normalizadas, 2 componentes cíclicos temporales, 50 pesos de texto TF-IDF por producto, y 5 columnas resultantes del CatBoost Target Encoding.
\end{itemize}
...
\subsection{Ingeniería de Variables Temporales Cíclicas}
Las horas del día ($H \in [0, 23]$) poseen una naturaleza cíclica. Forzar una escala lineal alteraría los algoritmos de distancia espacial. Por lo tanto, se aplicó una transformación matemática trigonométrica mapeando la variable temporal a un espacio bidimensional en el círculo unitario:
\begin{equation}
x_{\sin} = \sin\left(\frac{2\pi \cdot H}{24}\right), \quad x_{\cos} = \cos\left(\frac{2\pi \cdot H}{24}\right)
\end{equation}

\subsection{CatBoost Target Encoding para Alta Cardinalidad}
Para comprimir los identificadores categóricos discretos evitando la dispersión extrema de alternativas tradicionales como \textit{One-Hot Encoding}, se implementó un codificador basado en el cálculo de la probabilidad condicional de ocurrencia (Target Encoding), suavizado mediante un parámetro a priori para evitar el \textit{target leakage}:
\begin{equation}
...
El análisis de perfiles de los 71 registros catalogados como ruido desveló patrones de comportamiento transaccional aislados. Estos puntos corresponden a eventos con volúmenes de cantidad anómalos (por encima de 4 desviaciones estándar de la media de su propia categoría) combinados con marcas temporales en la madrugada (ventanas de baja densidad logística entre las 02:00 y las 04:30 horas). Al aislar geométricamente estos elementos atípicos, DBSCAN protegió de contaminación analítica a los vectores base del recomendador híbrido.
\end{techbox}
...
\end{equation}
El término del grafo $w_G = 0.00$ responde a una decisión técnica crítica: incorporar el PageRank directamente como una fuerza aditiva lineal saturaba las recomendaciones personalizadas con productos universales. Por consiguiente, se decidió desacoplar el PageRank del ensamble lineal y utilizarlo como un estimador independiente y contextual en el frontend, actuando como el indicador exclusivo del ``Pilar de la Despensa''.
\end{techbox}
...
\centering
\small
\caption{Matriz consolidada de evaluación offline}
\vspace{0.2cm}
\begin{tabular}{lccc}
\toprule
\textbf{Sistema Algorítmico Evaluativo} & \textbf{Precision@5} & \textbf{MAP@5} & \textbf{Cobertura (Coverage)} \\
\midrule
Popularity Baseline (Piso) & 0.0410 & 0.0480 & 22.00\% \\
PageRank Centrality (Global) & 0.0435 & 0.0530 & 26.00\% \\
Content-Based (TF-IDF) & 0.0380 & 0.0415 & 88.00\% \\
Collaborative Filtering (iALS) & 0.0485 & 0.0522 & 92.00\% \\
\textbf{Modelo Híbrido SKI (Producción)} & \textbf{0.0539} & \textbf{0.0574} & \textbf{100.00\%} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Análisis del Compromiso entre Precisión y Cobertura}
Al analizar los resultados aislados, sistemas estáticos globales como \textit{Popularity} o \textit{PageRank} exhiben métricas de precisión agregada competitivas debido a que explotan los productos de alta rotación comunes a todos los hogares; no obstante, su cobertura es críticamente deficiente (solo recomiendan el 22\% y 26\% del catálogo disponible). El \textbf{Modelo Híbrido SKI final supera de forma simultánea a todos los modelos de la competencia}, alcanzando el \textbf{MAP@5 más alto (0.0574)} y garantizando un \textbf{100.00\% de cobertura del catálogo}. Esto valida cuantitativamente la efectividad de la arquitectura multiclave para resolver las ineficiencias de suministro individuales sin descuidar el control del desperdicio perecedero.
