...
2. **Pruebas automatizadas limitadas a smoke tests.** `tests/` (35 pruebas, `pytest`) verifica shapes/rangos de los artefactos generados en cada etapa, pero no valida corrección numérica end-to-end (p. ej. no re-deriva las métricas de `evaluation_table.csv` desde cero) ni corre en un pipeline de CI — sigue dependiendo de ejecución manual local.
3. **Orquestador de pipeline sin paralelización.** `run_pipeline.py` reemplaza los 20+ comandos manuales de `runbook.md` por un único punto de entrada (con modos `--stage`/`--from`/`--skip-ingestion`), pero ejecuta las etapas secuencialmente como subprocesos; no paraleliza etapas independientes ni cachea resultados intermedios entre corridas.
